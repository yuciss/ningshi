...
- **Identity**: the kernel only answers "a process of a blocked uid appeared". Before signalling, the killer reads that process's own `/proc/<pid>/cmdline` and requires it to carry one of the package names the user blocked for that uid - a name the process itself was given, not something read out of a system file. The read is retried briefly, because Android names the process a moment after the credential switch. `/data/system/packages.list` is therefore only a lookup table for uids, never the authority on who a process is, so a uid recycled by a newly installed app can never be hit (such skips are counted in `status.gate.identity_skipped`).
- **Detection**: foreground / screen state / package-to-uid are read from kernel filesystems (cpuset / DRM / `packages.list`). A uid is `userId * 100000 + appId`, and the package list carries the appId, so the map is built for every user on the device: a rule key like `10:com.foo` resolves to the work profile instead of matching nothing. A pid's uid comes from `/proc/<pid>` ownership - one stat instead of parsing the status file of every process (verified identical on the test device).
- **Reporting**: a tgid is reported at most once per 5 seconds. A process that is already being handled (a blocked app in its last milliseconds, or one the identity check deliberately refuses) issues transactions far faster than anyone needs to hear about them - measured at ~800/s before this limit.
